#!/bin/bash
# 운영 2번 서버 (10.164.32.92) - 필수 파일 존재 확인

echo "=== 운영 2번 서버 필수 파일 확인 ==="

# 체크할 파일 및 디렉토리 목록
FILES_TO_CHECK=(
    "/opt/pg-auto-failover/config/cluster.conf"
    "/opt/pg-auto-failover/server2/01-install-all.sh"
    "/opt/pg-auto-failover/server2/02-setup-docker-container.sh"
    "/opt/pg-auto-failover/server2/03-start-service.sh"
    "/opt/pg-auto-failover/server2/04-stop-service.sh"
    "/opt/pg-auto-failover/server2/05-status-check.sh"
...
if [ ${#MISSING_FILES[@]} -eq 0 ]; then
    ...
    echo "다음 단계: ./server2/01-install-all.sh 실행"
else
